...
-- Task 12: Do employees with low environment satisfaction have higher attrition?
SELECT environment_satisfaction, COUNT(*) as emp_count,
SUM(CASE WHEN attrition ='Yes' THEN 1 ELSE 0 END) as employee_left,
ROUND(SUM(CASE WHEN attrition ='Yes' THEN 1 ELSE 0 END) *100 / COUNT(*),2) as attrition_rate
FROM hr_analytics
...
-- Task 3: write a query to find attrition rate by Education and Job level
SELECT department, job_role, education, job_level, COUNT(*) AS emp_count,
SUM( CASE WHEN attrition = 'Yes' THEN 1 ELSE 0 END ) AS employee_left,
ROUND(SUM( CASE WHEN attrition = 'Yes' THEN 1 ELSE 0 END ) *100 / COUNT(*),2) AS attrition_rate
FROM hr_analytics
GROUP BY department,job_role, education, job_level
ORDER BY employee_left DESC;

-- Task 4: Are younger employees more likely to leave?
...
-- Task 5: write a query to find is Work-Life Balance linked to Attrition?
...
-- Task 6: Who are the top "at-risk" employees based on low satisfaction, low income, high overtime, and no promotion?
...
-- Task 7: Which combination of factors (e.g., job role, overtime, income, satisfaction) is most associated with attrition?
...
-- Task 8; What patterns exist among high performers who left the company
